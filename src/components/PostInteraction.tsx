...
const PostInteraction = ({ post }: PostInteractionProps) => {
  ...
  const handleLike = () => {
    likeMutation.mutate(post.id, {
      onSuccess: () => {
        // Emit real-time notification if liked (not unliked)
        if (!post.isLiked && socket && user && post.user.id !== user.id) {
          socket.emit('like-post', {
            postId: post.id,
            userId: post.user.id,
            likedByUser: user.firstName || user.username || 'Someone'
          });
        }
      }
    });
  ...
  return (
    <div className="flex items-center justify-between gap-4 lg:gap-16 my-2 text-textGray">
      <div className="flex items-center justify-between flex-1">
        {/* comments */}
        <Link href={`/${post.user.username}/status/${post.id}`} className="flex items-center gap-2 cursor-pointer group">
          <svg
          ...
          <span className="group-hover:text-iconBlue text-sm">
            {post._count.directComments + post._count.comments}
          </span>
        </Link>
        ...
        <div className="flex items-center gap-2 cursor-pointer group">
          ...
          <span className="group-hover:text-iconGreen text-sm">0</span>
        </div>
        ...
        <div className="flex items-center gap-2 cursor-pointer group">
          ...
          >
            <path 
              className="fill-textGray group-hover:fill-iconBlue" 
              d="M12 2.59l5.7 5.7-1.41 1.42L13 6.41V16h-2V6.41l-3.29 3.3-1.41-1.42L12 2.59zM21 15l-.02 3.51c0 1.38-1.12 2.49-2.5 2.49H5.5C4.11 21 3 19.88 3 18.5V15h2v3.5c0 .28.22.5.5.5h12.98c .28 0 .5-.22.5-.5L19 15h2z"
            />
          </svg>
        </div>
      </div>
